...
keyword :: (RState state) => String -> ParserT state () [Char]
keyword x = string x *> spaces1

alphaNum :: (RState state) => ParserT state () Char
alphaNum = consume (\x -> isNumber x || isLetter x) "alphaNum"

identifier :: (RState state) => ParserT state () [Char]
identifier = many1 alphaNum <* spaces

separator :: (RState state) => Char -> ParserT state () Char
separator x = char x <* spaces
